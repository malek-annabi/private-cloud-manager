export function Button({
  ...
  return (
    <button
      ...
      disabled={disabled}
      title={title}
      className={`inline-flex items-center justify-center rounded-2xl px-4 py-2 text-sm font-medium transition-all duration-300 ease-smooth active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:shadow-none hover:-translate-y-0.5 ${classes[variant]} ${className}`}
    >
      {children}
    </button>
  );
